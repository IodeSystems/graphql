...
// TestParseDocumentErrorSkipEOF is deliberately absent.
//
// Upstream covers parseDocument's `if skp, err := skip(parser, lexer.EOF);
// err != nil` branch by stubbing Parser.LexToken with a function that always
// fails. This fork removed that field in c62226b, so the parser lexes through
// lexer.ReadToken directly and there is no seam to inject through.
//
// The branch is not reachable with real input either: skip advances only when
// the current token is already EOF, and advancing from EOF re-reads EOF, which
// cannot error. Input like `{a} "` does produce a lex failure, but it surfaces
// from item(parser) at parser.go:249, not from the skip -- verified by
// coverage, which reports the skip branch at 0.
//
// Porting it would mean either restoring the injection field or asserting a
// pass that comes from the wrong branch. Neither is worth it for a branch that
// is dead in this fork.
